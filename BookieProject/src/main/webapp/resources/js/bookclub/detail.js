							...
							$
									.ajax({
										...
										success : function(comment) {
											...
											commentInput.value = "";
											let ul = document
													.getElementById('com_ul').lastElementChild;
											if (ul == null) {
												ul = document
														.getElementById('com_ul');
											}
											const li = document
													.createElement('li');
											...
											const div4 = document
													.createElement('div');
											const updateAtag = document
													.createElement('a');
											updateAtag.setAttribute("class",
													"commentUp");
											updateAtag.setAttribute("id",
													comment.id);
											updateAtag.style.cssText = "font-size: 12px;";
											updateAtag.innerText = "수정";
											div4.append(updateAtag);

											const deleteAtag = document
													.createElement('a');
											deleteAtag.setAttribute("class",
													"commentDel");
											deleteAtag.setAttribute("id",
													comment.id);
			...
			.on(
					...
					function() {
						...
						const nickname = document
								.getElementsByClassName('comment_nickname')[0].innerHTML;
						const inner = "<div class='temp'><div class='writer comment_nickname'>"
								+ nickname
								+ "</div><form name='editForm'><label for='submit' class='label_summit'>"
								+ "<textarea rows='1' name='comment' id='update_comment' class='comment_input_temp' placeholder='댓글을 수정하세요'></textarea>"
								+ " <input type='submit' class='comment_submit'value='등록' name='submit'></label></form></div>";
						if (toggle_bool) {
								...
								.on(
										...
										function() {
											const comment = document
													.getElementById('update_comment').value;
											if (comment != "") {
												$
														.ajax({
	...
	$(document)
			.on(
					...
					function() {
						...
						const inner = "<div class='temp'><div class='writer comment_nickname'>"
								+ nickname
								+ "</div><form name='replyForm'><label for='submit' class='label_summit'>"
								+ "<textarea rows='1' name='comment' id='reply_comment' class='comment_input_temp' placeholder='댓글을 작성하세요'></textarea>"
								+ "<input type='submit' class='reply_submit' value='등록' name='submit'></label></form></div>";
						if (toggle_bool) {
							$('#div_' + commentId).eq(0).after(inner);
							toggle_bool = false;
						} else { // toggle event
								...
								.on(
										...
										function() {
											const reply = document
													.getElementById('reply_comment').value;
											if (reply != "") {
												$
														.ajax({
															url : "/club/reply",
	...
	$(document)
			.on(
					...
					function() {
						...
						const inner = "<div class='temp'><div class='writer comment_nickname'>"
								+ nickname
								+ "</div><form name='editReplyForm'><label for='submit' class='label_summit'>"
								+ "<textarea rows='1' name='comment' id='update_reply' class='comment_input_temp' placeholder='댓글을 수정하세요'></textarea>"
								+ " <input type='submit' class='comment_submit'value='등록' name='submit'></label></form></div>";
						if (toggle_bool) {
						...
						$('form[name=editReplyForm]')
								.on(
										'submit',
										function() {
											const comment = document
													.getElementById('update_reply').value;
											if (comment != "") {
												$
														.ajax({
